...
def generate_huffman_lookup(weights_table):
    ...
    for key_hex, weight in weights_table.items():
        try:
            # Превращаем строковый ключ "0x0041" в число 0x41
    ...
    # Слияние узлов до получения единого корня
    while len(heap) > 1:
    ...
    # ИСПРАВЛЕНО: забираем единственный оставшийся корневой узел из кучи
...
def decode_zone1_coordinates(zone1_bytes, weights_table, max_points=516):
    ...
    """
    # 1. Генерируем lookup-таблицу на лету
    ...
    for bit in bit_string:
        ...
        if current_bits in lookup:
            ...
            # Нам нужно собрать ровно 516 пар (1032 значения X и Y), после чего останавливаемся
            if len(coordinates_stream) >= (max_points * 2):
                break
                
    # 4. Группируем плоский массив восстановленных чисел в пары координат (X, Y)
    points = []
...
# --- ВХОДНЫЕ ДАННЫЕ ДЛЯ ЗАПУСКА СИСТЕМЫ ---

# Полная извлеченная таблица весов (93 элемента из вашего среза 0x00A6 - 0x21A)
...
# Оригинальный бинарный дамп Зоны 1 (до начала текста)
...
# Автоматически отрезаем Зону 1 строго до начала текста "kolymskaya"
text_marker = b'\x04`H \xf0\xf9\xc2<X@\x0fN'
zone1_end_index = full_dump_data.find(text_marker)
zone1_bytes_clean = full_dump_data[:zone1_end_index]

# Запуск комплексной декомпрессии географического массива
restored_points = decode_zone1_coordinates(zone1_bytes_clean, extracted_weights_table, max_points=516)

# Вывод результатов разбора
print(f"--- РЕЗУЛЬТАТЫ ДЕКОМПРЕССИИ ЗОНЫ 1 ---")
print(f"Размер чистых байт Зоны 1 для анализа: {len(zone1_bytes_clean)} байт.")
print(f"Успешно извлечено и сгруппировано геометрических точек: {len(restored_points)}")
print("\nПервые 15 восстановленных координатных пар (X, Y):")
for idx, point in enumerate(restored_points[:15], start=1):
    print(f"  Точка {idx:02d}: X = {point[0]:<5} (0x{point[0]:04X}) | Y = {point[1]:<5} (0x{point[1]:04X})")
