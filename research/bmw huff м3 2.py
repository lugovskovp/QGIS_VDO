...
def generate_full_huffman_lookup(weights_table):
    """
    Строит полное дерево Хаффмана для всех 256 байт (0x00 - 0xFF).
    Возвращает инвертированную мапу: { 'бинарный_код_строкой': целочисленный_байт }
    """
...
def execute_final_psf_decompress(zone1_bytes, weights_table, total_points=516):
    # 1. Генерация полной 256-байтовой lookup-мапы
    lookup = generate_full_huffman_lookup(weights_table)
    
    # 2. Разворачивание физического буфера Зоны 1 в биты
    bit_string = "".join(f"{byte:08b}" for byte in zone1_bytes)
    
    unpacked_bytes = bytearray()
    current_bits = ""
    required_bytes_count = total_points * 4 # 516 * 4 = 2064 байта
    
    # Побитовый проход по всей длине стрима
    ...
    # 3. Валидация правила чётности High-байта (индексы 0, 2, 4, 6... <= 0xC0)
    ...
    # 4. Сборка абсолютных 16-битных координат (Big-Endian UInt16)
    absolute_points = []
    ptr = 0
    ...
    # Печать комплексного отчета финального теста
    print("=====================================================")
    print("=== ФИНАЛЬНЫЙ ТЕСТ ДЕКОМПРЕССИИ И ВЕРИФИКАЦИИ PSF ===")
    print("=====================================================")
    print(f"Физический размер битовой ленты Зоны 1: {len(bit_string)} бит")
    print(f"Распаковано байт из потока Хаффмана:   {len(unpacked_bytes)} из {required_bytes_count}")
    print(f"Сформировано абсолютных точек (X, Y):   {len(absolute_points)} из {total_points}")
    
    if verified_even_rule:
        print("Валидация чётных High-байтов (<= 0xC0):  🟢 УСПЕШНО ПОДТВЕРЖДЕНА")
    else:
        print(f"Валидация провалена: 🔴 СБОЙ на чётной позиции {failed_index} (значение: 0x{unpacked_bytes[failed_index]:02X} > 0xC0)")
    print("=====================================================\n")
    
    return absolute_points


# --- ТАБЛИЦА ВЕСОВ И ДАННЫЕ ИЗ ВАШЕГО ФАЙЛА КАРТЫ ---
...
# Запуск сквозного конвейера декомпрессии
final_points = execute_final_psf_decompress(zone1_full_bytes, extracted_weights_table, total_points=516)

if final_points:
    print("Пример первых 10 восстановленных абсолютных точек (X, Y):")
    for i, pt in enumerate(final_points[:10], start=1):
        print(f"  Точка {i:02d}: X = {pt[0]:<5} (0x{pt[0]:04X}) | Y = {pt[1]:<5} (0x{pt[1]:04X})")


"""
'0x8A': 1,    # Однобайтовый управляющий токен ADAS
 То, что токен 0x8A имеет минимальный вес 1, в алгоритме Хаффмана означает, что он уходит на самую глубокую ветку и получает длинный префикс. В гео-форматах PSF это классический Escape-маркер (управляющий токен).
