...
def decode_zone1_with_high_weight_escapes(zone1_bytes, weights_table, max_bytes=2064):
    ...
    # Новые триггеры переключения (высокочастотные токены 0x17 и 0x19)
    HIGH_WEIGHT_ESCAPES = [0x17, 0x19]
    ...
    try:
        for bit in bit_iterator:
            ...
            if current_bits in lookup:
                ...
                if token in HIGH_WEIGHT_ESCAPES:
                    # ПОЙМАЛИ НАСТОЯЩИЙ ESCAPE: извлекаем следующие 8 бит (1 сырой байт) напрямую
                    raw_bits = "".join(next(bit_iterator) for _ in range(8))
                    raw_byte = int(raw_bits, 2)
                    unpacked_bytes.append(raw_byte)
                else:
                    # Обычный токен из таблицы весов
...
# --- ТАБЛИЦА ВЕСОВ И СТРИМ ИЗ ДАМПА ---
...
final_result_bytes = decode_zone1_with_high_weight_escapes(zone1_full_bytes, extracted_weights_table, max_bytes=2064)

print(f"--- РЕЗУЛЬТАТ РАСПАКОВКИ С ВЫСОКОЧАСТОТНЫМИ ESCAPE-ТОКЕНАМИ ---")
print(f"Всего байт извлечено в итоговый массив: {len(final_result_bytes)} из 2064.")


"""
Поиск кандидатов в вашей таблице весов
Если мы берем за основу оригинальный неполный словарь из 93 элементов, то идеальной парой для такой схемы 
являются токены:0x8A (вес 1) — самый редкий токен расширенного бинарного диапазона.0x0A (вес 6) или 0x00 
(если он есть в заголовке, вес 12) — маркеры системных префиксов.Либо, если мы смотрим на пару высокочастотных 
токенов, которые кодируются рекордно короткими префиксами в 2 бита:0x17 (вес 372) \(\rightarrow \) 
код 010x19 (вес 352) \(\rightarrow \) код 11Один из них (например, 0x17) может быть маркером «Прочитай 1 сырой байт», 
а второй (0x19) — маркером «Прочитай 2 сырых байта (WORD)».
"""