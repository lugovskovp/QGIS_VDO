...
def decode_zone1_dual_escapes(zone1_bytes, weights_table, max_bytes=2064):
    """
    Распаковывает поток Хаффмана, используя дуальные Escape-маркеры:
    - При токене 0x17 извлекает из итератора строго следующие 8 бит (1 байт)
    - При токене 0x19 извлекает из итератора строго следующие 16 бит (2 байта / WORD)
    """
    ...
    # Назначаем дуальные маркеры на основе весов
    BYTE_ESCAPE = 0x17  # Триггер чтения 1 байта
    WORD_ESCAPE = 0x19  # Триггер чтения 2 байт (WORD)
    ...
    try:
        for bit in bit_iterator:
            ...
            if current_bits in lookup:
                ...
                if token == BYTE_ESCAPE:
                    # Извлекаем 1 сырой байт (8 бит)
                    b_bits = "".join(next(bit_iterator) for _ in range(8))
                    unpacked_bytes.append(int(b_bits, 2))
                    
                elif token == WORD_ESCAPE:
                    ...
                    # Обычный однобайтовый токен (или дельта)
        ...
        # Поток бит корректно исчерпан
        pass
        
    return unpacked_bytes

# --- ТАБЛИЦА ВЕСОВ И СТРИМ ИЗ ВАШЕГО ДАМПА ---
...
# Передайте ваши 1394 байта Зоны 1
zone1_full_bytes = b'...' 
...
# Запуск дуального декодера
dual_flat_bytes = decode_zone1_dual_escapes(zone1_full_bytes, extracted_weights_table, max_bytes=2064)

print(f"--- РЕЗУЛЬТАТ ДУАЛЬНОЙ ESCAPE-РАСПАКОВКИ (BYTE + WORD) ---")
print(f"Всего байт извлечено в итоговый массив: {len(dual_flat_bytes)} из 2064.")
