...
def test_ch_idx_exact_offsets(all_vdo_fixture):
...
def test_ch_idx_cp1250_encoding(all_vdo_fixture):
...
def test_ch_idx_init_too_small_buffer(all_vdo_fixture):
...
def test_ch_idx_repr(all_vdo_fixture):
    ...
    assert ' out:' in repr(idx)


def test_ch_idx_property_caching(all_vdo_fixture):
...
def test_ch_idx_slots_integrity(all_vdo_fixture):
